...
svg::Color JsonReader::ParseColor(const json::Node& node) {
    ...
    if (components.size() == 4) {
        return svg::Rgba(components[0].AsInt(), components[1].AsInt(), components[2].AsInt(),
                         components[3].AsDouble());
    }
...
json::Node JsonReader::BuildBusResponse(int request_id, std::string_view name,
    ...
    return json::Builder{}
        .StartDict()
            ...
            .Key("stop_count"s).Value(static_cast<int>(stat->stops_on_route))
