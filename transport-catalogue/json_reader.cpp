...
void JsonReader::ProcessBaseRequests(const json::Node::Array &base_requests) {
  ...
  for (const auto &item : base_requests) {
    ...
    if (type_it->second.AsString() == "Stop") {
      ...
      if (name_it == obj.end() || stops_it == obj.end() ||
          roundtrip_it == obj.end()) {
        continue;
      }

      std::string bus_name = name_it->second.AsString();
      bool is_roundtrip =
          roundtrip_it->second.IsBool() ? roundtrip_it->second.AsBool() : false;
...
json::Node::Array
JsonReader::ProcessStatRequests(const json::Node::Array &stat_requests) {
  ...
  for (const auto &request : stat_requests) {
    ...
    std::string type =
        type_it->second.IsString() ? type_it->second.AsString() : "";
    std::string name =
        name_it->second.IsString() ? name_it->second.AsString() : "";
    ...
    if (type == "Bus") {
      ...
      if (bus_stat) {
        result["stop_count"] =
            json::Node(static_cast<int>(bus_stat->stop_count));
        result["unique_stop_count"] =
            json::Node(static_cast<int>(bus_stat->unique_stop_count));
        result["route_length"] =
            json::Node(static_cast<int>(bus_stat->route_length));
