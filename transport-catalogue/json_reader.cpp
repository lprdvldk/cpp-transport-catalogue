...
JsonReader::JsonReader(database::transport_catalogue::TransportCatalogue& catalogue) : catalogue_(catalogue) {
}
...
void JsonReader::LoadStops(const json::Array& base_requests) {
    for (const json::Node& node : base_requests) {
        ...
        database::geo::Coordinates coords{request.at("latitude"s).AsDouble(), request.at("longitude"s).AsDouble()};
...
void JsonReader::LoadStopDistances(const json::Array& base_requests) {
    for (const json::Node& node : base_requests) {
        ...
        const database::Stop* from = catalogue_.FindStop(request.at("name"s).AsString());
        ...
        for (const auto& [stop_name, distance_node] : distances_it->second.AsDict()) {
...
renderer::RenderSettings JsonReader::ParseRenderSettings(const json::Document& doc) {
    ...
    settings.bus_label_offset = svg::Point(bus_offset.at(0).AsDouble(), bus_offset.at(1).AsDouble());
    ...
    settings.stop_label_offset = svg::Point(stop_offset.at(0).AsDouble(), stop_offset.at(1).AsDouble());
...
json::Node JsonReader::BuildBusResponse(int request_id, std::string_view name, const RequestHandler& handler) const {
...
json::Node JsonReader::BuildStopResponse(int request_id, std::string_view name, const RequestHandler& handler) const {
...
json::Node JsonReader::BuildMapResponse(int request_id, const RequestHandler& handler) const {
    return json::Builder{}
        .StartDict()
        .Key("request_id"s)
        .Value(request_id)
...
json::Node JsonReader::BuildResponse(const json::Dict& request, const RequestHandler& handler) const {
    ...
    if (type == "Bus"s) {
        return BuildBusResponse(request_id, request.at("name"s).AsString(), handler);
    }
    if (type == "Stop"s) {
        return BuildStopResponse(request_id, request.at("name"s).AsString(), handler);
    }
    if (type == "Map"s) {
...
json::Document JsonReader::ProcessStatRequests(const json::Document& doc, const RequestHandler& handler) const {
    const json::Dict& root = doc.GetRoot().AsDict();
