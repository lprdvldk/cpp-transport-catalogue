...
std::unordered_map<const Bus*, svg::Color>
MapRenderer::AssignBusColors(const std::vector<const Bus*>& sorted_buses) const {
...
std::map<std::string_view, const Stop*>
MapRenderer::CollectUniqueStops(const std::vector<const Bus*>& sorted_buses) const {
...
void MapRenderer::RenderRouteLines(svg::Document& doc, const SphereProjector& projector,
...
void MapRenderer::AddLabelPair(svg::Document& doc, svg::Point position, const std::string& text, svg::Point offset,
...
void MapRenderer::RenderBusLabels(svg::Document& doc, const SphereProjector& projector,
                                  const std::vector<const Bus*>& sorted_buses,
                                  const std::unordered_map<const Bus*, svg::Color>& bus_colors) const {
    for (const Bus* bus : sorted_buses) {
        if (bus->route.empty()) {
...
void MapRenderer::RenderStopCircles(svg::Document& doc, const SphereProjector& projector,
                                    const std::map<std::string_view, const Stop*>& unique_stops) const {
    for (const auto& entry : unique_stops) {
        const Stop* stop = entry.second;
        svg::Circle circle;
        circle.SetCenter(projector(stop->coords)).SetRadius(settings_.stop_radius).SetFillColor(std::string("white"));
        doc.Add(std::move(circle));
    }
}

void MapRenderer::RenderStopLabels(svg::Document& doc, const SphereProjector& projector,
                                   const std::map<std::string_view, const Stop*>& unique_stops) const {
    for (const auto& entry : unique_stops) {
        const Stop* stop = entry.second;
...
svg::Document MapRenderer::RenderRouteMap(const std::vector<const Bus*>& sorted_buses) const {
