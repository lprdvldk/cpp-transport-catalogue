...
class MapRenderer {
 public:
  ...
  svg::Document RenderRouteMap(
      const std::vector<const database::transport_catalogue::Bus*>&
          sorted_buses) const;
 ...
 private:
  ...
  void AddLabelPair(svg::Document& doc, svg::Point position,
                    const std::string& text, svg::Point offset, int font_size,
                    const svg::Color& label_fill, bool bold) const;
};
