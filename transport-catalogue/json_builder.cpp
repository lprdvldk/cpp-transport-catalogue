...
Builder::Builder() { nodes_stack_.push_back(&root_); }

Node& Builder::AddValue(Node node) {
  if (nodes_stack_.empty()) {
    throw std::logic_error(
        "Adding a value to an already completed object is not allowed"s);
  }
  ...
  if (top->IsDict()) {
    throw std::logic_error(
        "Key() must be called before a value inside a Dict"s);
  }
...
KeyItemContext Builder::Key(std::string key) {
  ...
  if (!top->IsDict()) {
    throw std::logic_error(
        "Key() is allowed only right after StartDict() or after a value inside a Dict"s);
  }
